...
namespace kcp2k
{
    public abstract class KcpPeer
    {
        ...
        // 用于接收KCP处理后的消息的缓冲区（避免分配内存）。
        // 重要：这是用于KCP消息的，所以需要的大小为：
        //      1字节头部 + 最大消息内容长度
        readonly byte[] kcpMessageBuffer;// = new byte[1 + ReliableMaxMessageSize];

        // 用于将用户消息交给 kcp 进行处理的发送缓冲区（避免分配内存）。
        ...
        // 可靠通道（即 kcp）允许的最大消息大小，让外部能知道可发送的最大消息尺寸。
        // Send() 方法中的计算并不直观，所以在这里提供辅助方法。
        //
        // kcp 支持分片，所以最大消息远大于 MTU。
        //
        // -> 运行时 MTU 修改被禁止：mss 总是 MTU_DEF-OVERHEAD
        // -> Send() 检查分片数量 < rcv_wnd，因此这里用 rcv_wnd - 1
        //    注意：原版 kcp 存在一个 bug，即使用默认的 WND_RCV 而不是配置的 rcv_wnd，导致最大消息受到限制（144 KB）。
        //    https://github.com/skywind3000/kcp/pull/291
        //    本库 kcp2k 已修复该问题。
        // -> 每个消息还需加 1 字节 KcpHeader 枚举头，所以再减 1
        //
        // 重要提示：最大消息实际为 MTU * rcv_wnd，换句话说会完全填满接收窗口！
        ...
        protected void Reset(KcpConfig config)
        {
            ...
            // 其中 METADATA_SIZE_RELIABLE = 5， 即高层协议自己的包头长度为5字节。
            // 因此，需要将高层 MTU - 5 传递给 kcp.SetMtu，保证最后完整包不超过 config.Mtu
