    ...
    public class KcpServer
    {
        ...
        static Socket CreateServerSocket(bool DualMode, ushort port)
        {
            if (DualMode)
            {
                ...
                // 关掉UDP Socket 收到 ICMP 端口不可达后被系统关闭的行为
                //                 问题场景：
                // 服务器用一个 UDP socket 服务多个客户端。
                // 某个客户端关掉了，服务器再往那个地址发包。
                // Windows 可能回一个 ICMP Port Unreachable。
                // 默认情况下，这个 ICMP 会让整个 UDP socket 被 OS 关掉。
                // 之后 Receive/Send 就会抛异常，其它还活着的客户端也一起受影响。
                // SIO_UDP_CONNRESET + { 0x00 } 的作用：关闭这种 “收到 ICMP 就 reset/关闭 socket” 的默认行为，让 socket 继续工作。
